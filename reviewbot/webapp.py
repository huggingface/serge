...
def _launch_task_pod(job: Job, worker_cfg: Config, req: TaskRequest) -> None:
    ...
    try:
        ...
        if cfg.task_execution == "docker":
            ...
            if mirror_bare:
                # Bind-mount the repo's bare mirror read-only as the fetch seed.
            ...
            # Mount the mirror PVC read-only (per-repo subPath) only when a claim
            # is configured AND the repo's bare mirror exists.
            mount_claim = (
