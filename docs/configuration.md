...
| `TOOL_REPEAT_LIMIT` | `tool_repeat_limit` | `3` | Byte-identical tool calls tolerated before the loop is cut off. Each repeat is told it is repeating, and a repeat that returned the same bytes is served as a pointer to the copy already in the transcript rather than a second copy. `0` disables. |
